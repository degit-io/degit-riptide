...
export class HttpDuplex extends EventEmitter {
  ...
  /**
   * Requested HTTP Version sent by the server. Usually either '1.0' or '1.1'
  ...
  /**
   * The HTTP status code. Generally assigned before sending headers for a response to a server.
  ...
  /**
   * Controls the status message sent to the server as long as an explicit call to response.writeHead() isn't made
   ...
   * after the initial response to the server.
  ...
  /**
   * Sends a response header to the server request. Must only be called one time and before calling response.end().
